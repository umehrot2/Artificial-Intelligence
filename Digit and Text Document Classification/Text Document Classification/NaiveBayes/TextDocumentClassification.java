...
import java.util.*;
/**
 * Created by uditmehrotra on 04/04/15.
 */
public class TextDocumentClassification
{
    ...
    //Loads Training and Test data from file into appropriate data srtuctures along with class labels
    ...
    //This function builds the model for the classifier, which can be used for prediction later
    private void classifier()
    {
        //Classifier HashMap stores words as keys, and value is another Hashmap which will store the frquency of that word for each class label (class label is the key).
        
        classifier = new HashMap<String,HashMap<Integer, Double>>();
        training_labels_frequency = new HashMap<Integer, Integer>(); //Stores frequencies of each label
        training_labels_words_frequency = new HashMap<Integer, Integer>(); //Stores total word counts corresponding to each class label
        ...
        //adding labels which might not have occurred for a particular Word, and add count as 0 for that word-class label pair
            ...
            for (Integer class_label : training_labels_frequency.keySet())
            {
                ...
                for (String key : test_data.get(loop).keySet())
                {
                    ...
                    if(!applyLaplace)
                    {
                        ...
                        //Adding the count of attributes unique words to denominator for Laplacian correction
    ...
    //This function computes classification rate, confusion matrix, confusion pairs, words with highest log-odd ratios
    private void classificationRate(ArrayList<String> unique_words)
    {
        ...
        System.out.println("--------------------------------------");
        for(Integer label : training_labels_frequency.keySet()) {
            System.out.println("Classification Accuracy for class " + label + " : " + df2.format((class_correct_classified.get(label) / (test_label_frequency.get(label) * 1.0))));
        }
        System.out.println("--------------------------------------\n");
    ...
    //This function computes top 20 highest frequency words
    ...
    public static void main(String[] args)
    {
        ...
        TextDocumentClassification obj = new TextDocumentClassification();
