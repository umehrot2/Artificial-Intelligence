...
public class PacPoint implements Comparable<PacPoint>
{
    ...
    public boolean equals(Object o) {
        ...
        /*
        int goals1 = c.goals.size();
        if(c.goals.contains(c.p))
        {
            goals1 = goals1 - 1;
        }

        int goals2 = goals.size();
        if(goals.contains(goals2))
        {
            goals2 = goals2 - 1;
        }*/

        //return p.x == c.p.x && p.y == c.p.y && goals1 == goals2 //&& (c.pathCost + c.heuristic == heuristic + pathCost);
        // Compare the data members and return accordingly
        return p.x == c.p.x && p.y == c.p.y && goals.size() == c.goals.size();

        //return p.x == c.p.x && p.y == c.p.y && (c.pathCost + c.heuristic == heuristic + pathCost);
    }
