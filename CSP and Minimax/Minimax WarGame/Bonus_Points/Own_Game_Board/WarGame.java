...
class assignment
{
    ...
    assignment(int rows, int cols)
    {
    	assignment = new char[rows][cols];
...
//    static String fileName = "Narvik.txt";
    ...
    public assignment applyMove(assignment a, Move m, char player)
    {
        ...
        assignment as = new assignment(numRows, numCols);
        ...
        else
        {
            ...
            //down
            if(i+1 < numCols && as.assignment[i+1][j] == opponent)
            {
                as.assignment[i+1][j] = player;
            }
            ...
            //right
            if(j+1 < numRows && as.assignment[i][j+1] == opponent)
            {
                as.assignment[i][j+1] = player;
            }
        }
        ...
        for(int i = 0; i < a.assignment.length; i++)
        {
            for(int j = 0; j < a.assignment[0].length; j++)
            {
                if(a.assignment[i][j] == ' ')
                {
                    ...
                    //down
                    else if(i+1 < numCols && a.assignment[i+1][j] == player)
                    {
                        isBlitz = true;
                    }
                    ...
                    //right
                    else if(j+1 < numRows && a.assignment[i][j+1] == player)
                    {
                        isBlitz = true;
                    }
    ...
    public static void getRowsCols(String fileName){
    	try
        {
            ...
            numRows = row;
            numCols = col;
            System.out.println("Number of rows: "+numRows+" Number of Cols: "+numCols);
//            System.exit(0);
    ...
    public void minimax_vs_minimax(assignment a)
    {
        ...
        for(int loop = 0; loop < numTimes; loop++)
        {
            ...
            {
                max_depth = depth_max;
                if(numTimes - loop < max_depth)
                {
                    max_depth = numTimes - loop;
                }
                nodes_expanded = 0;
    ...
    private void alphabeta_vs_alphabeta(assignment a)
    {
        ...
        for(int loop = 0; loop < numTimes; loop++)
        {
            ...
            if(loop % 2 == 0)
            {
                ...
                if(numTimes - loop < max_depth_alpha_beta)
                {
                    max_depth_alpha_beta = numTimes - loop;
                }
...
//        System.out.println("Average Time taken per move in Milliseconds: " + df.format(average_time_taken));
    ...
    {
//    	String[] runFiles = {"seven.txt", "eight.txt", "nine.txt", "ten.txt"};
   	    String[] runFiles = {"seven.txt", "eight.txt"};
    	// String[] runFiles = {"ten.txt"};
    	long startTime = 0;
    	long stopTime = 0;
    	for(int zz = 0; zz < runFiles.length; zz++){
    		
//	    	getRowsCols(fileName);
	    	getRowsCols(runFiles[zz]);
	        ...
	        System.out.println("File name is "+runFiles[zz]);
	        game.readFile(runFiles[zz]);
	        System.out.println("File: " + runFiles[zz]);
	        System.out.println("Minimax Depth: " + depth_max);
	        System.out.println("Alpha Beta Minimax Depth: " + depth_max_alpha_beta);
	        game.displayBoard();
	//        System.exit(0);
	        assignment a = new assignment(numRows, numCols);
	        /*double avg_time = 0.0;
	        for(int i = 0; i < 10; i++) {
	            long startTime = System.currentTimeMillis();
	            System.out.println("\nMINIMAX vs. MINIMAX: ");
	        ...
	        System.out.println("===========File is "+runFiles[zz]+"==========");
	        System.out.println("\nMINIMAX vs. MINIMAX: ");
	        
	        startTime = System.currentTimeMillis();	        
	        game.minimax_vs_minimax(a);
	        stopTime = System.currentTimeMillis();
            System.out.println("MINIMAX vs. MINIMAX Time " + (int) (stopTime - startTime));
	
//	        System.out.println("\nALPHA BETA vs. ALPHA BETA: ");
//	        startTime = System.currentTimeMillis();
//	        game.alphabeta_vs_alphabeta(a);
//	        stopTime = System.currentTimeMillis();
//            System.out.println("ALPHA BETA vs. ALPHA BETA:  Time " + (int) (stopTime - startTime));
//	
//	        System.out.println("\nMINIMAX vs. ALPHA BETA: ");
//	        startTime = System.currentTimeMillis();
//	        game.minimax_vs_alphabeta(a);
//	        stopTime = System.currentTimeMillis();
//            System.out.println("MINIMAX vs. ALPHA BETA:  " + (int) (stopTime - startTime));
//            
//	        System.out.println("\nALPHA BETA vs. MINIMAX: ");
//	        startTime = System.currentTimeMillis();
//	        game.alphabeta_vs_minimax(a);
//	        stopTime = System.currentTimeMillis();
//            System.out.println("ALPHA BETA vs. MINIMAX: Time " + (int) (stopTime - startTime));
//	        break;
    	}
    }
}
